...
from transformers import AutoTokenizer, PretrainedConfig, AutoProcessor, CLIPModel, ViTImageProcessor, ViTModel
...
def center_crop(image, size):
    return ImageOps.fit(image, size, method=0, bleed=0.0, centering=(0.5, 0.5))
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help="A list of images corresponding to the validation prompts list that is used for validation.",
    )
    parser.add_argument(
        "--validation_epochs",
        type=int,
...
def main(args):
    ...
    clip_model = CLIPModel.from_pretrained("openai/clip-vit-large-patch14") # TODO make this an argument
        ...
        if accelerator.is_main_process:
            if args.validation_prompts is not None and epoch % args.validation_epochs == 0:
                logger.info(
                    ...
                    f" {', '.join(args.validation_prompts)}."
                )
                # create pipeline
                if not args.train_text_encoder:
                    text_encoder_one = text_encoder_cls_one.from_pretrained(
